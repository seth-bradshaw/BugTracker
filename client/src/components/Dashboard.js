...
import {
  filterTicketsByNotStarted,
  filterTicketsByInProgress,
...
export default function Dashboard() {
  ...
  const displayAllTickets = useSelector(
    state => state.tickets.displayAllTickets
  );
  ...
  return (
    <>
      <Container textAlign="center" style={{ marginTop: '20px' }}>
        ...
        {notStartedStatus
          ? tickets
              .filter(ticket => ticket.ticket.status === 'Not Started')
              .map(ticket => {
                return (
                  <>
                    <EditTicket
        ...
        {inProgressStatus
          ? tickets
              .filter(ticket => ticket.ticket.status === 'In Progress')
              .map(ticket => {
                return (
                  <>
                    <EditTicket
        ...
        {completedStatus
          ? tickets
              .filter(ticket => ticket.ticket.status === 'Completed')
              .map(ticket => {
                return (
                  <>
                    <EditTicket
          ...
          : null}
        <button onClick={() => push('/companyemployees')}>
          COMPANY EMPLOYEES TEMP BUTTON
        </button>
      </Container>
      <TestTicketBoard />
    </>
