...
export default function PutTicket() {
    ...
    const putTicketRequest = (newTicket) => {
        ...
        axiosWithAuth()
            .put(`/company/4/ticket/12`, newTicket)
            .then(res => {
    ...
    return (
        <div className="ticket-form-container">
            ...
            <Form className="ticket-form" onSubmit={() => putTicketRequest(editTicket)}>
                <Form.Field width="16">
                    <label>Title</label>
                    <input type="text" value={editTicket.title} placeholder="Title" name="title" onChange={changeHandler}/>
                </Form.Field>
                <Form.Group>
                    <Form.Field>
                        <label>ErrorCode</label>
                        <input type="text" value={editTicket.errorCode} placeholder="Error Code" name="errorCode" onChange={changeHandler}/>
                    </Form.Field>
                    <Form.Field>
                        <label>Error Category</label>
                        <input type="text" value={editTicket.errorCategory} placeholder="Error Category" name="errorCategory" onChange={changeHandler}/>
                    </Form.Field>
                    <Form.Field width="5">
